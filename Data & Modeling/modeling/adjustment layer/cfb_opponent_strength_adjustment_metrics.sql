...
with cfb_opponent_strength_adjustment_dim as (
    select
      ...
      , sum(a.receiving_touchdowns) / nullif(sum(a.receptions), 0)            as adj_from_receiving_touchdowns_per_reception
      , sum(b.receiving_touchdowns) / nullif(sum(b.receptions), 0)            as adj_to_receiving_touchdowns_per_reception
        -- RUSHING
      ...
      , sum(a.rushing_touchdowns) / nullif(sum(a.rush_attempts), 0)           as adj_from_rushing_touchdowns_per_rush
      , sum(b.rushing_touchdowns) / nullif(sum(b.rush_attempts), 0)           as adj_to_rushing_touchdowns_per_rush
        -- PASSING
      ...
      , sum(b.passing_touchdowns) / nullif(sum(b.completions), 0)             as adj_to_passing_touchdowns_per_completion
...
-- to / from  = ratio (2 is  to > from! 0.5 is from > to.
   ...
   , cfb_opponent_strength_adjustment_regression as (
    select
      ...
      , regr_slope(ln(nullif(receiving_touchdowns_per_reception_delta, 0)), true_opp_elo_delta)     as rec_tds_slope
      , regr_intercept(ln(nullif(receiving_touchdowns_per_reception_delta, 0)), true_opp_elo_delta) as rec_tds_intercept
        -- RUSHING
      ...
      , regr_slope(ln(nullif(rushing_touchdowns_per_rush_delta, 0)), true_opp_elo_delta)            as rush_tds_slope
      , regr_intercept(ln(nullif(rushing_touchdowns_per_rush_delta, 0)), true_opp_elo_delta)        as rush_tds_intercept
        -- PASSING
      ...
      , regr_slope(ln(nullif(passing_touchdowns_per_completion_delta, 0)), true_opp_elo_delta)      as pass_tds_slope
      , regr_intercept(ln(nullif(passing_touchdowns_per_completion_delta, 0)), true_opp_elo_delta)  as pass_tds_intercept
