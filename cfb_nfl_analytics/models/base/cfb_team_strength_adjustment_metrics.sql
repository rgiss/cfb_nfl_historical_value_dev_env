...
-- Converted from: /Users/riley.gisseman/Downloads/cfb_nfl_dev_env/Data & Modeling/modeling/adjustment layer/cfb_team_strength_adjustment_metrics.sql
...
with cfb_team_strength_adjustment_dim as (
    select
      ...
      , avg(a.epa + a_opp_adj.epa_factor) / avg(a.team_snaps)                                                             as adj_from_epa_per_snap
      , avg(b.epa + b_opp_adj.epa_factor) / avg(b.team_snaps)                                                             as adj_to_epa_per_snap
        -- RECEIVING
    ...
    from {{ ref("cfb_game_logs") }}                                       as a
         inner join {{ ref("cfb_game_logs") }}                            as b
         ...
         inner join {{ ref("cfb_opponent_strength_adjustment_metrics") }} as a_opp_adj
         ...
         inner join {{ ref("cfb_opponent_strength_adjustment_metrics") }} as b_opp_adj
...
-- to / from  = ratio (2 is  to > from! 0.5 is from > to.
   ...
   , cfb_team_strength_adjustment_regression as (
    select
      ...
      , regr_slope((epa_delta), true_team_elo_delta)                                                               as epa_slope
      , regr_intercept((epa_delta), true_team_elo_delta)                                                           as epa_intercept
        -- RECEIVING
   ...
   , cfb_team_strength_adjustment_metrics as (
    select distinct
      ...
      , ((2500 - spine.team_elo) * epa_slope + epa_intercept)                               as epa_factor
        -- RECEIVING
