...
-- Converted from: /Users/riley.gisseman/Downloads/cfb_nfl_dev_env/Data & Modeling/modeling/base/cfb_game_logs.sql
-- cfb_game_logs
...
with game_logs_union as (
    ...
    from {{ source("raw", "college_football_play_by_play_data") }}          as n
         left join {{ ref("stg_ncaaf_season_games_regularization") }} as reg on reg.pos_team = n.pos_team and reg.game_id = n.game_id
         inner join {{ ref("stg_clean_cfb_player_names") }}           as cpn
    ...
    from {{ source("raw", "college_football_play_by_play_data") }}          as n
         left join {{ ref("stg_ncaaf_season_games_regularization") }} as reg on reg.pos_team = n.def_pos_team and reg.game_id = n.game_id
         inner join {{ ref("stg_clean_cfb_player_names") }}           as cpn
...
from game_logs_union                      as glu
     ...
     left join {{ source("raw", "cfb_elo_data") }}               as opp_elo on opp_elo.team = glu.opponent and opp_elo.year = glu.year and opp_elo.week = case
                                                                                                                                       when glu.week = 1
                                                                                                                                           then 13
                                                                                                                                           else glu.week
                                                                                                                                       end
